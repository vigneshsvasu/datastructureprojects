...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        while (parent > 0 && p.myPriority > getNode(index).myPriority) {
            swap(parent, index);
            index = parent;
            parent = index / 2;
            p = getNode(parent);
    ...
    private void sink(int index) {
        // Throws an exception if index is invalid. DON'T CHANGE THIS LINE.
        validateSinkSwimArg(index);
        double x = getNode(index).priority();
        double y = getNode(index * 2).priority();
        double z = getNode(2 * index + 1).priority();
        if(x > y && x > z) {
            if (y < z) {
                swap(index, index * 2);
                sink(index * 2)
